<template>
  <div class="app-container">
    ...
    <!-- 视频背景 - 可滚动扩大的区域 -->
    <section class="relative w-full h-screen overflow-hidden transition-all duration-1000" ref="heroSection" :style="heroStyle">
      <!-- 动态背景 -->
      <div class="absolute inset-0 bg-black/50 z-10 transition-opacity duration-700" :style="{ opacity: 1 - scrollProgress * 0.8 }"></div>
      <div class="absolute inset-0 z-0 w-full h-full bg-cover bg-center" :style="{ backgroundImage: 'url(https://picsum.photos/id/10/1920/1080)', transform: `scale(${1.1 + scrollProgress * 0.3})` }"></div>

      <!-- 文字介绍 - 居中显示 -->
      <div class="absolute inset-0 z-20 flex flex-col justify-center items-center text-center px-4 transition-opacity duration-700" :style="{ opacity: 1 - scrollProgress }">
      ...
      <!-- 向下滚动指示器 -->
      <div class="absolute bottom-8 left-0 right-0 z-20 flex justify-center animate-bounce transition-opacity duration-700" :style="{ opacity: 1 - scrollProgress * 2 }">
...
const handleScroll = () => {
  ...
  // 当滚动进度达到100%时，固定hero区域并重置滚动位置
  if (newProgress >= 1 && !isScrolling.value) {
    isScrolling.value = true
    isHeroExpanded.value = true
    
    // 调整页面布局，使hero固定且内容区域显示在下方
    setTimeout(() => {
...
const handleWheel = (e) => {
  if (!isHeroExpanded.value) {
    ...
    let newProgress = Math.min(Math.max(scrollProgress.value + scrollDelta, 0), 1)
...
const handleTouchMove = (e) => {
  if (!isHeroExpanded.value && touchStartY.value) {
    e.preventDefault()

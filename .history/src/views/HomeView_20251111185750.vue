...
const handleScrollAnimation = (progress) => {
  ...
  // 视频容器展开动画 - 确保全屏覆盖且左右边缘精确对齐
  const videoContainer = document.getElementById('videoContainer')
  if (videoContainer) {
    // 计算缩放比例，确保视频左右边缘精确对齐浏览器窗口
    const initialWidth = '60vw'
    const initialHeight = '40vh'
    
    // 使用CSS变换进行高性能缩放，避免重排
    gsap.to(videoContainer, {
      // 计算缩放值
      width: `${60 + 40 * clampedProgress}vw`,
      height: `${40 + 60 * clampedProgress}vh`,
      borderRadius: `${(1 - clampedProgress) * 0.5}rem`,
      duration: 0.05, // 非常短的持续时间以获得60fps的响应性
      ease: 'power2.out',
      // 确保transform重置以避免冲突
...
const handleWheel = (e) => {
  ...
  // 确保足够的更新频率但不过度消耗资源
  if (deltaTime < 8) return // 约120fps的更新频率，确保平滑

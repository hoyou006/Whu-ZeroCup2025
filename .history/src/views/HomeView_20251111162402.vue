<template>
  <div class="app-container">
    ...
    <nav id="navbar" class="fixed top-0 left-0 right-0 z-[1000]">
      ...
      <!-- 视频背景 - 可滚动扩大的区域 -->
      <section class="relative w-full h-screen overflow-hidden" ref="heroSection" style="background-image: url(/img/homePage/bgHome.jpg); background-size: cover; background-position: center; background-repeat: no-repeat; transition: background-image 1s ease;">
        <!-- 背景图片直接应用在section上 -->
...
const handleScrollAnimation = (progress) => {
  ...
  if (videoContainer) {
    gsap.to(videoContainer, {
      ...
      left: clampedProgress >= 1 ? 0 : 'auto',
      top: clampedProgress >= 1 ? 0 : 'auto',
      position: clampedProgress >= 1 ? 'fixed' : 'relative',
      zIndex: 10,
      duration: 0.5,
  ...
  if (heroSection.value) {
    if (clampedProgress > 0.5) {
      // 当滚动超过一半时，开始淡出背景图片并显示背景色
      const opacity = 1 - 2 * (clampedProgress - 0.5)
      heroSection.value.style.backgroundImage = `linear-gradient(to bottom, rgba(32, 104, 100, ${1 - opacity}), rgba(32, 104, 100, ${1 - opacity})), url(/img/homePage/bgHome.jpg)`
    } else {
      // 恢复原始背景图片

...
const completeExpansion = () => {
  ...
  if (videoContainer) {
    gsap.to(videoContainer, {
      ...
      position: 'fixed',
      zIndex: 10,
      duration: 0.8, // 延长动画时间以获得更平滑的效果
      ease: 'power2.out',
      // 确保绝对定位并重置所有可能影响全屏的样式
      margin: 0,
      padding: 0,
      border: 'none',
  ...
  if (animationController.value.isExpanded && window.scrollY < window.innerHeight * 0.5) {
    ...
    if (videoContainer) {
      gsap.to(videoContainer, {
        ...
        position: 'relative',
        zIndex: 10,
        duration: 1.2, // 延长动画时间以获得更流畅的倒放效果
        ease: 'power2.inOut',
        // 添加缩放和位移动画，使倒放效果更明显
        scale: 1,
        x: 0,
